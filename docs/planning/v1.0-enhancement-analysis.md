# mcp-audit v1.0 Feature Enhancement Analysis for v1.x → v2.0
...
Based on comprehensive analysis of mcp-audit's 19 core modules and 41+ capabilities against December 2025 market trends, **8 major feature areas** require substantial enhancement to remain competitive and address emerging needs in the MCP ecosystem.
...
2. **Platform Fragmentation** - 10+ major AI coding tools vs mcp-audit's 3 platforms
...
### Files to Modify
- `src/mcp_audit/goose_adapter.py` (new)
- `src/mcp_audit/cursor_adapter.py` (new)
- `src/mcp_audit/cline_adapter.py` (new)
- `src/mcp_audit/aider_adapter.py` (new)
- `src/mcp_audit/base_tracker.py` (adapter registration)
- `src/mcp_audit/cli.py` (platform detection)
...
**Market Opportunity:**
- mcp-audit is uniquely positioned: has tool call data, can analyze patterns
...
- `src/mcp_audit/security/` (new directory)
- `src/mcp_audit/security/tool_scanner.py` (new)
- `src/mcp_audit/security/toxic_flow.py` (new)
- `src/mcp_audit/security/credential_detector.py` (new)
- `src/mcp_audit/smells.py` (add security smell patterns)
...
### Enhancement Plan (v1.1.0 - v1.3.0)

| Version | Feature | Priority | Description |
|---------|---------|----------|-------------|
| v1.1.0 | **Ollama Tokenizer** | P1 | Local model token estimation |
...
### Files to Modify
- `src/mcp_audit/token_estimator.py` (add Ollama support)
- `src/mcp_audit/ollama_adapter.py` (new)
- `src/mcp_audit/base_tracker.py` (context window tracking)
...
### Files to Modify
- `src/mcp_audit/pricing_config.py` (budgets, thresholds)
- `src/mcp_audit/pricing_api.py` (enterprise integrations)
- `src/mcp_audit/cli.py` (budget commands)
...
### Files to Modify
- `src/mcp_audit/smells.py` (add security/cost/governance patterns)
- `src/mcp_audit/recommendations.py` (security recommendations)
- `src/mcp_audit/smell_aggregator.py` (cross-category aggregation)
...
- Directory: `~/.mcp-audit/sessions/<platform>/<date>/`
...
### Files to Modify
- `src/mcp_audit/storage.py` (pluggable backends)
- `src/mcp_audit/aggregation/` (new directory)
- `src/mcp_audit/export/governance.py` (new)
...
### Files to Modify
- `src/mcp_audit/display/rich_display.py` (security panel)
- `src/mcp_audit/display/session_browser.py` (comparison mode)
- `src/mcp_audit/display/dashboard.py` (new)
...
### Files to Modify
- `src/mcp_audit/base_tracker.py` (new dataclasses)
...
**This analysis identifies 8 major feature areas requiring substantial enhancement, with security analysis (P0) and platform expansion (P0) as the most critical gaps between mcp-audit v1.0 and market requirements.**
